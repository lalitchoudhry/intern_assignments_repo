  ...
  return (
    <div className="w-screen h-screen bg-lightGreen flex justify-center items-center">
      ...
      >
        ...
        <button className="bg-green text-white p-3 font-semibold w-full rounded-lg hover:text-green hover:bg-white border-green border-2" onClick={handleSubmit}>
          Submit
        </button>
        {isRegistered ? (
          <span>
            Already have an account ?
            <a onClick={toggleRegistered} className="text-blue hover:underline cursor-pointer">Log in</a>
          </span>
        ) : (
          <span>
            New user ?
            <a onClick={toggleRegistered} className="text-blue hover:underline cursor-pointer">Registser here</a>
          </span>
        )}
      </form>
    </div>
